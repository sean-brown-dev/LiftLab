...
class DoubleProgressionCalculatorTests {
    ...
    @Test
    fun `weight should not change when all set goals are met for a standard lift`() {
        val lift = WorkoutLiftWithRelationships(
            workoutLift = WorkoutLift(
                workoutId = 0,
                liftId = 0,
    ...
    fun `weight should increment by increment override when all myorep match set goals are met`() {
        ...
        val previousSetData = listOf(
            ...
            MyoRepSetResultDto(workoutId = 0, liftId = 0, reps = 1, rpe = 8f, setPosition = 0, myoRepSetPosition = 2, weight = 75f, microCycle = 0),
        )
