...
/*
    ...
    fun `recalculate when top set exceeds rep range top within allowed RPE`() {
        ...
        stubWeightCalcReturns(listOf(100f, 10f, 5f), calls)

        // here (constructor below) => not the deload week, current-week target reps via step sequence should be 7
        val result = WaveLoadingProgressionCalculator(programDeloadWeek = 4)
            .calculate(liftEntity.toCalculationDomainModel(), previous, previous, false)

        // Increment should flatten across sets despite first one being recalculated because last rec is used
        result.forEach { set ->
            assertEquals(80f, set.weightRecommendation)
        }
        assertEquals(1, calls.size)
    }

    @Test
    fun `do NOT recalc when intermediate is too easy, but not over goal`() {
        val liftEntity = WorkoutLiftWithRelationships(
            workoutLiftEntity = WorkoutLiftEntity(
        ...
        val previous = listOf(
            // top set fine
            StandardSetResult(workoutId = 0, liftId = 0, reps = 8, rpe = 7.5f, liftPosition = 0, setPosition = 0, weight = 75f, setType = SetType.STANDARD, isDeload = false),
            // intermediate (8 + 2RIR) < goal (8 + 1RIR + 1.5) buffer, so no recalc
            StandardSetResult(workoutId = 0, liftId = 0, reps = 8, rpe = 8f, liftPosition = 0, setPosition = 1, weight = 75f, setType = SetType.STANDARD, isDeload = false),
            // final fine
            StandardSetResult(workoutId = 0, liftId = 0, reps = 8, rpe = 10f, liftPosition = 0, setPosition = 2, weight = 75f, setType = SetType.STANDARD, isDeload = false),
        )
    ...
    fun `single set - exceeding top within allowed RPE triggers recalculation (not simple +increment)`() {
        ...
        val calls = mutableListOf<CalcCall>()
        stubWeightCalcReturns(listOf(90f), calls)
        ...
        // Exceed -> recalc
        result.forEach { set ->
            assertEquals(90f, set.weightRecommendation)
        }
        assertEquals(1, calls.size)
    ...
    fun `when current week equals repRangeTop, exceeding top should recalc instead of decrementing for new microcycle`() {
        // with deloadWeek=4 and stepSize=1 => sequence [8,7,6] before deload; => target reps == 6 (repRangeBottom)
        ...
        val calls = mutableListOf<CalcCall>()
        stubWeightCalcReturns(listOf(80f, 85f, 90f), calls)

        // Force "isTopOfRepRange == true" by making so current-week reps == repRangeTop
        val result = WaveLoadingProgressionCalculator(programDeloadWeek = 4)
            .calculate(liftEntity.toCalculationDomainModel(), previous, previous, false)

        // Because we exceeded top, the branch should do RECALC (not "decrementForNewMicrocycle")
        result.forEach { set ->
            assertEquals(90f, set.weightRecommendation) { "Should not decrement for new microcycle when exceeding top" }
        }
