...
class HistoricalWorkoutNamesSyncRepository(
...
) : BaseSyncRepository<HistoricalWorkoutNameFirestoreEntity, HistoricalWorkoutNameEntity>(
    dao = dao,
    ...
    override suspend fun getAll(): List<HistoricalWorkoutNameFirestoreEntity> =
        dao.getAll().map { it.toFirestoreDto() }

    override suspend fun getMany(ids: List<Long>): List<HistoricalWorkoutNameFirestoreEntity> =
